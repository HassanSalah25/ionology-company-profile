    ...
    <div class="row justify-content-center align-items-center clearfix" style="height: 100%">
        <div class="body col-md-9">
            <div class="row clearfix">
                <form class="col-md-12" method="POST" action="{{  route('dashboard.'.$route.'.update', $category->id) }}"
                        ...
                        <div class="card">
                            ...
                            <div class="body">
                                <input type="file" name="image" class="dropify"
                                       id="dropify-event"
                                       data-url-remove="{{route('dashboard.'.$route.'.removeImage',$category->id)}}"
                                       data-default-file="{{$category->getMedia('images')->first()?->getUrl()}}">
                            </div>
                        </div>
                    </div>
                            ...
                            <div role="tabpanel" class="tab-pane in {{$loop->first ? 'active' : ''}}"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="ckeditor" name="description[{{$language->code}}]"
                                              placeholder="Description"/>{{$category->getTranslation('description',$language->code)}}</textarea>
                                </div>
                                <hr/>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        <textarea class="ckeditor" name="meta_description[{{$language->code}}]"
                                                  class="form-control"
                                                  placeholder="Meta Description"/>{{$category->seo->getTranslation('meta_description',$language->code)}}</textarea>
                                    </div>
                                </div>
                            </div>
                        @endforeach
